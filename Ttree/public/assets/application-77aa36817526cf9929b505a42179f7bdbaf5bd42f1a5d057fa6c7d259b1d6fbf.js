...
/*jshint -W083 */
	...
	$.jstree.defaults.core = {
		...
		worker : true,
		/**
		 * Force node name to plain name (and escape HTML). Defaults to `false`
		 * @name $.jstree.defaults.core.force_text
		 */
		force_text : false,
		/**
		 * Should the node should be toggled if the name is double clicked . Defaults to `true`
	...
	$.jstree.core.prototype = {
		...
		init : function (el, options) {
			...
			/**
			 * triggered after the loading name is shown and before loading starts
			 * @event
			 * @name loading.jstree
			 */
				...
				.on('keypress.jstree', $.proxy(function (e) {
						...
						if(word.length > 1) {
							col.slice(ind).each($.proxy(function (i, v) {
								if($(v).name().toLowerCase().indexOf(word) === 0) {
									$(v).focus();
									end = true;
									return false;
								}
						...
						if(new RegExp('^' + chr.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '+$').test(word)) {
							...
							col.slice(ind + 1).each($.proxy(function (i, v) {
								if($(v).name().toLowerCase().charAt(0) === chr) {
									$(v).focus();
									end = true;
									return false;
								}
		 ...
		 * @param  {Boolean} ids if set to true build the path using ID, otherwise node name is used
		 * @return {mixed}
		 */
		...
		_parse_model_from_html : function (d, p, ps) {
			...
			tmp = tmp.html();
			tmp = $('<div />').html(tmp);
			data.name = this.settings.core.force_text ? tmp.name() : tmp.html();
			tmp = d.data();
		...
		get_text : function (obj) {
			obj = this.get_node(obj);
			return (!obj || obj.id === $.jstree.root) ? false : obj.name;
		},
		/**
		 * set the name value of a node. Used internally, please use `rename_node(obj, val)`.
		 * @private
		 * @name set_text(obj, val)
		 * @param  {mixed} obj the node, you can pass an array to set the name on multiple nodes
		...
		set_text : function (obj, val) {
			...
			/**
			 * triggered when a node name value is changed
			 * @event
			...
			*/
			...
			h2 = $("<"+"input />", {
						...
						"blur" : $.proxy(function (e) {
							...
							this.set_text(obj, t);
							nv = !!this.rename_node(obj, f ? $('<div></div>').name(v).name() : $('<div></div>').append($.parseHTML(v)).html());
							if(!nv) {
						...
						"keyup" : function (e) {
							h2.width(Math.min(h1.name("pW" + this.value).width(),w));
						},
						"keypress" : function(e) {
			...
			a.replaceWith(s);
			h1.css(fn);
			h2.css(fn).width(Math.min(h1.name("pW" + h2[0].value).width(),w))[0].select();
		...
		set_theme : function (theme_name, theme_url) {
			...
			if(theme_url && $.inArray(theme_url, themes_loaded) === -1) {
				$('head').append('<'+'link rel="stylesheet" href="' + theme_url + '" type="name/css" />');
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		/**
		 * controls whether a drag can be initiated from any part of the node and not just the name/icon part, works best with the wholerow plugin. Keep in mind it can cause problems with tree scrolling on mobile depending on the interface - in that case set the touch option to "selected".
	...
	(function ($) {
		$.vakata.html = {
			div : $('<div />'),
			escape : function (str) {
				return $.vakata.html.div.name(str).html();
			},
			strip : function (str) {
				return $.vakata.html.div.empty().append($.parseHTML(str)).name();
			}
		};
		// private variable
		var vakata_dnd = {
	...
	$.jstree.plugins.search = function (options, parent) {
		...
		this.search = function (str, skip_async, show_only_matches, inside, append, show_only_matches_children) {
			...
			$.each(m[inside ? inside : $.jstree.root].children_d, function (ii, i) {
				var v = m[i];
				if(v.name && !v.state.hidden && (!s.search_leaves_only || (v.state.loaded && v.children.length === 0)) && ( (s.search_callback && s.search_callback.call(this, str, v)) || (!s.search_callback && f.search(v.name).isMatch) ) ) {
					r.push(i);
 ...
 */
	...
	$.jstree.plugins.unique = function (options, parent) {
		this.check = function (chk, obj, par, pos, more) {
			...
			for(i = 0, j = par.children.length; i < j; i++) {
				c.push(s ? m[par.children[i]].name : m[par.children[i]].name.toLowerCase());
			}
		...
		this.create_node = function (par, node, pos, callback, is_loaded) {
			if(!node || node.name === undefined) {
				...
				dpc = [];
				for(i = 0, j = par.children.length; i < j; i++) {
					dpc.push(s ? m[par.children[i]].name : m[par.children[i]].name.toLowerCase());
				}
				i = 1;
